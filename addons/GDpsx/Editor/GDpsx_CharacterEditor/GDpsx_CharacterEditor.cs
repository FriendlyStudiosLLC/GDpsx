...
public partial class GDpsx_CharacterEditor : Control
{
	...
	public void GetCharacterSceneData(string path)
	{
		if (Engine.IsEditorHint())
		{
			...
			{
				foreach (Node child in charSceneRoot.GetChildren())
				{
					child.QueueFree();
				}
			}
			character3DScene = (PackedScene)GD.Load(path);
			Node charScene = character3DScene.Instantiate();
	...
	{
		GDpsx_Character _tempChar = new GDpsx_Character();
		...
		foreach (GDpsx_Character character in editor.gameBrain.Characters)
		{
			int charIndex = editor.gameBrain.Characters.IndexOf(character);
	...
	{
		foreach (GDpsx_Character _char in editor.gameBrain.Characters)
		{
	...
	public void UpdateBrain()
	{
		...
		foreach (Node child in characterList.GetChildren())
		{
			child.QueueFree();
		}
		...
		foreach (GDpsx_Character character in editor.gameBrain.Characters)
		{
			...
			GDpsx_CharacterButton button = charButton.Instantiate() as GDpsx_CharacterButton;
