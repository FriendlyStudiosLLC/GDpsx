using System;
using GDpsx_Project.addons.GDpsx.Game.Scripts.First_Person;
using Godot;
...
namespace GDpsx_Project.addons.GDpsx.Game.Scripts
{
	...
	{
		[ExportCategory("Health Settings")] [Export]
		...
		[ExportCategory("Movement Variables")] [Export]
		...
		[ExportCategory("Interaction Settings")] [Export]
		public RayCast3D LookAtCast;

		public Node3D LookedAtObject = null;


		[ExportCategory("Misc Settings")] [Export]
		public float LerpSpeed { private set; get; } = 10.0f;
		...
		[Export] public Inventory.GDpsx_Inventory Inventory;
		...
		public Node3D PerformLookAt()
		{
			if (LookAtCast.IsColliding())
			{
				if (LookAtCast.GetCollider() is IInteractable)
				{
					...
					if (LookedAtObject is First_Person.GDpsx_GameObject)
					{
						GDpsx_GameObject LookAtGameObject = LookedAtObject as First_Person.GDpsx_GameObject;
